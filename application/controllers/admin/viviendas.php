...
class Viviendas extends MY_Controller {
    ...
    public function editarInspeccion($idv = 0, $orden = 0, $ciclo = 0) {
        ...
        if ($submit == "Guardar") {
            ...
            $data['positivas'] = $this->vivienda->getPositivasByCicloYViviendaYOrden($data['item']->ciclo, $data['item']->id_vivienda,$data['item']->id_orden);
     ...
     * @return void
     */
    public function delPos($id_inspeccion, $idv, $idPos, $ciclo_orden) {
        $id_inspeccion = (int) $id_inspeccion;
        $idv = $idv;
        $idPos = (int) $idPos;
        $this->vivienda->delPos($idPos);
        redirect(base_url() . 'admin/viviendas/editarInspeccion/' . $idv . '/' . $id_inspeccion . "/" . $ciclo_orden, 'location');
    }
     ...
     * @return view
     */
    public function crea($idvivienda) {
        $submit = $this->input->post('submit');
