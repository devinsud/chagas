...
class Vivienda extends CI_Model {
	...
	public function edicion_inspeccion() {
		...
		foreach ($et as $eti) {
			$intraperi = $this->getIntraPeri($eti);
			$inpe      = (isset($intraperi[0]))?$intraperi:'';
			if ($inpe != '') {
	...
	public function getViviendasByBarrio($id) {
		$id = (int) $id;
		$this->db->select('viviendas.id_vivienda as idv,viviendas.id as id, viviendas_inspeccion.ciclo, viviendas.tipo as tipo, viviendas.id_sede as id_sede, latitud, longitud, id_barrio, id_orden');
		$this->db->from('viviendas');
		$this->db->distinct();
		$this->db->join('viviendas_inspeccion', 'viviendas_inspeccion.id_vivienda = viviendas.id_vivienda', 'left');
	...
	public function guardaLista() {
		...
		$fech  = ($u['fecha'] == '')?date('Y-m-d'):$u['fecha'];
		$ff    = explode('-', $fech);
